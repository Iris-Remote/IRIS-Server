...
async fn get_result(req:web::Json<GetTaskResu>) -> impl Responder {
    if req.auth != AUTHTOKEN{
        ...
        match result {
            ...
            None => return HttpResponse::NotFound().body("not received"),
        }
...
async fn add_result(data:String) -> impl Responder {
    ...
    if clear == "base64_decode_error" || clear == "invalid_data"  || clear == "decryption_error"{
        ...
        let task: AddTaskresult = match serde_json::from_str(&clear){
            Ok(task) => task,
            Err(_) => {
                println!("Missmatch");
                return "invalid".to_string();
...
async fn get_task_with_timeout(device_id: &str) -> String {
    let timeout = Duration::from_secs(10);
    let start = Instant::now();

    loop {
        if let Some((command, taskid)) = gettask(device_id.to_string()) {
            let combined = format!("{}__{}", command, taskid);
            let encrypted =
                crypt::encrypt_string(ENCRYPTKEY.as_bytes(), &combined);
...
async fn advertise_device(data:String) -> String{
    ...
    return get_task_with_timeout(device_id.as_str()).await;
